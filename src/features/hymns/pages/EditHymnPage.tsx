/**
 * Página para editar un himno existente.
 * Ruta: /hymn/:id/edit  (protegida — admin o editor)
 */
...
import { editHymn, ApiError } from '@/core/api';
import { useHymnDetail } from '../hooks';
import { LoadingSpinner } from '@/components/ui/LoadingSpinner';
import { ErrorMessage } from '@/components/ui/ErrorMessage';
import { Toast } from '@/components/ui';

export function EditHymnPage() {
  const { id } = useParams<{ id: string }>();
  const hymnId = id ?? '';
  const numericId = parseInt(hymnId, 10);
  ...
  // Pre-populate form once hymn data is loaded
  useEffect(() => {
    if (hymn !== null && !initialized) {
      setTitle(hymn.title);
      setRawContent(hymn.content);
  ...
  const handleSubmit = async (e: SyntheticEvent<HTMLFormElement>) => {
    ...
    if (user === null || token === null) {
      setErrorMsg('Debes iniciar sesión para editar himnos.');
      return;
    }

    if (isNaN(numericId)) {
      setErrorMsg('ID de himno inválido.');
      return;
    }
    ...
    try {
      await editHymn(numericId, {
        title: title.trim(),
        content,
        user: { email: user.email, name: user.name },
      }, token);
      setToastVisible(true);
      setTimeout(() => { navigate(`/hymn/${hymnId}`, { replace: true }); }, 1500);
    } catch (err) {
  ...
  return (
    <article>
      ...
      {/* Estado de carga del himno */}
      {loadingState === 'loading' && <LoadingSpinner message="Cargando himno..." />}

      {/* Error al cargar el himno */}
      {loadingState === 'error' && error !== null && (
        <ErrorMessage message={error.message} onRetry={retry} />
      )}

      {/* Formulario (visible una vez que el himno cargó) */}
      ...
      <Toast message="Himno actualizado con éxito" visible={toastVisible} />
    </article>
  );
}
